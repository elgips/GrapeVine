...
%     Pmin=[0.7,0.65];
%     Pmax=[1.2,0.65];
    P1Plant=linspace(Pmin(1),Pmax(1),4);
...
%% soil paraneters
    ...
    ParanumSoil=ListSoilPara(ParamSoilNames,28,SolXml);
    PnomSoil=SoilParaNominal(ParanumSoil,28,SolXml);
    PminSoil=ParMin(SolXml,ParanumSoil);
    PmaxSoil=ParMax(SolXml,ParanumSoil);
    space=5;
    Valmtrix=Range2Vec( Pmin,Pmax,space);
%     points=10^2;
%     Pmin=[0.7,0.65];
%     Pmax=[1.2,0.65];
    P1Soil=linspace(PminSoil(1),PmaxSoil(1),4);
    %%
    ...
    for i=1:n1
        for j=1:n2
            ...
            TreatInitLr(LfRVec,TecXml);
             ...
             [Y,~,~,T]=RunChanges(ParanumPlant,ParanumSoil,USM_name,PlantXmlNom,PlantXmlVar,SoilXml,PPlant(ii),PSoil(ii),SoilNameVar);
