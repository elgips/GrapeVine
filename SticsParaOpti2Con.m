function F = SticsParaOpti2Con(ParanumPlant,ParanumSoil,ParamVals,Pmin,Pmax)
%A function that get a list of parameters and return an objective function
%value for fminsearch function, based on stics model runs, with constraint panelty. and SWP2SWF parameter 
%   INPUT -SOIL parameters list, plant parameter list
...
%                Er=Er2sr(Erd,i,j,l);
               ...
               Q=Q+sum(Residual.^2)/max(Residual)/length(Residual);
        end
    end
...
% end
F=Q+(1-PinRange2(ParamVals,Pmin,Pmax))*100;%(ParamVals(1)-1.2).^2+(ParamVals(2)-39).^2;
disp(F);
end
